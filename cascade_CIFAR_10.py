...
parser.add_argument('--bits', default='4,8,16', type=str,
...
parser.add_argument('--max-iter', default=41, type=int,
    ...
    for iter in range(max_iter):
        ...
        for epoch in range(epochs):
            for iteration, (train_input, train_label, batch_ind) in enumerate(trainloader):
                ...
                model.zero_grad()
                loss = 4 * adsh_loss(code, code_length, V, S, V[batch_ind.cpu().numpy(), :]) + \
                       2 * adsh_loss(code1, code_length1, V1, S, V1[batch_ind.cpu().numpy(), :]) + \
                       adsh_loss(code2, code_length2, V2, S, V2[batch_ind.cpu().numpy(), :])
                loss.backward()
                optimizer.step()
        ...
        logger.info('[Iteration Time: %3d]', iter_time)
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/log-MAH-cifar10', datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
