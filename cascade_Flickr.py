...
parser.add_argument('--bits', default='8,10,12', type=str,
                    help='binary code length (default: 12,24,32,48)')
parser.add_argument('--gpu', default='2', type=str,
                    help='selected gpu (default: 1)')
parser.add_argument('--arch', default='cascade', type=str,
...
def encoding_onehot(target, nclasses=21):
...
def _dataset():
    ...
    dset_database = dp.DatasetProcessingFlickr(
        'data/Flickr-25', 'database_image.txt', 'database_label.txt', transformations
    )
    dset_test = dp.DatasetProcessingFlickr(
        'data/Flickr-25', 'test_image.txt', 'test_label.txt', transformations
    )
    ...
    databaselabels = load_label('database_label.txt', 'data/Flickr-25')
    testlabels = load_label('test_label.txt', 'data/Flickr-25')
    ...
    for iter in range(max_iter):
        ...
        for epoch in range(epochs):
            for iteration, (train_input, train_label, batch_ind) in enumerate(trainloader):
                ...
                model.zero_grad()
                loss1 = 6 * adsh_loss(code, code_length, V, S, V[batch_ind.cpu().numpy(), :])
                loss2 = 2 * adsh_loss(code1, code_length1, V1, S, V1[batch_ind.cpu().numpy(), :])
                loss3 = adsh_loss(code2, code_length2, V2, S, V2[batch_ind.cpu().numpy(), :])
        ...
        logger.info('[Iteration: %3d/%3d][Train Loss: %.4f][2# Train Loss: %.4f][3# Train Loss: %.4f]', iter, max_iter, loss_, loss1_, loss2_)
        ...
        if iter % 10 == 0:
            ...
            record['topkpre'].append(topkpre)
            record['topkpre'].append(topkpre1)
            record['topkpre'].append(topkpre2)
            model.train()
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/log-MAH-flickr', datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
